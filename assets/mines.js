...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-d4fd91ea";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-d4fd91ea">'+'<span class="tile-face-d4fd91ea tile-front-d4fd91ea"></span>'+'<span class="tile-face-d4fd91ea tile-back-d4fd91ea"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-d4fd91ea")){return;}
var back=tile.querySelector(".tile-back-d4fd91ea");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-d4fd91ea");if(isMine){tile.classList.add("boom-d4fd91ea");}
if(dim){tile.classList.add("dimmed-d4fd91ea");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-d4fd91ea")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-d4fd91ea");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-d4fd91ea")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-d4fd91ea");void panelEl.offsetWidth;panelEl.classList.add("jolt-d4fd91ea");}
endRound();}
